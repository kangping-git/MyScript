...
function error(errorCode, system) {
    ...
    console.error(
        `Error (l:${system.lines + 1}) SC${errorC}:${errors[errorC]}`
    );
}
// MEMO Lexer Function
function lexer(code) {
    return code
        .split(
            /("[^"]*"|[0-9]+\.[0-9]+|[0-9]+|[a-zA-Z_][a-zA-Z0-9_]*|\(|\)]*|\n|\*\*|\+|-|\*|\/)|\r|;| /
        )
        .filter((v) => v);
}
...
function runner(
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let variableData = {
            ...
            import: async (name) => {
                try {
                    await import("./module/" + name + ".js").then((md) => {
                        for (let i in md) {
                            variableData[i] = md[i];
                        }
                    });
                    ...
                    if (ast2.length != 1) {
                        ...
                        while (true) {
                            if (
                                ...
                                let temp = (yield runAst([ast2[i]], vars)).r;
                                let temp2 = (yield runAst([ast2[i + 2]], vars))
                                    .r;
                                if (
                                    typeof temp == "number" &&
                                    typeof temp2 == "number"
